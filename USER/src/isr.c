...
//        // 等待接收
//        receive_char = uart_getchar(UART_6);
        // 查询接收
        uart_query( UART_6 , &receive_char);
        // 发送接收的字符串到串口助手
        uart_putchar( UART_6 , receive_char);
        if(receive_char == 'A')
        {
            RoadFlag.barn_in_flag = 1; //前车
        }
...
void EXTI4_IRQHandler(void)
{
    if(SET == EXTI_GetITStatus(EXTI_Line4))
    {
        ...
        RoadTask.fork_rl = 0;
        ips114_show_string(0, 0, "right");


//        flash_data_union_buffer[2].int32_type  = key;                        //2
//
//        if(flash_check(FLASH_SECTION_62, FLASH_PAGE_3)==1)                      // 判断是否有数据   //3
//        {
//            flash_erase_page(FLASH_SECTION_62, FLASH_PAGE_3);                // 擦除这一页
//        }
//
//        flash_write_page_to_data_buffer(FLASH_SECTION_62, FLASH_PAGE_3);         //4
